  ...
  projectCards.forEach((card) => {
    card.addEventListener("click", () => {
      ...
      projectCards.forEach((c) => {
        ...
        c.querySelector(".project-content").style.opacity = "0";
      });
  ...
  carousels.forEach((carousel) => {
    ...
    for (let i = 1; i <= maxImages; i++) {
      ...
      let p = new Promise((resolve) => {
        img.onload = () => {
          img.className =
            "w-full h-full flex-shrink-0 object-cover pointer-events-none";
    ...
    Promise.all(promises).then(() => {
      ...
      if (loadedImages.length === 0) {
        ...
        setupCarouselControls(carousel, loadedImages.length);
      }
    });
  });

  function setupCarouselControls(carousel, totalSlides) {
    const track = carousel.querySelector(".carousel-track");
